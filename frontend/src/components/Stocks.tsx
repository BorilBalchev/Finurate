...
import { topStocks } from './stocks_data';

import { addStockToPortfolio, type Asset } from './Add_asset';
...
const Stocks = () => {
    ...
    const raw = topStocks.find(stock => stock.ticker === selected);
    const selectedStock: Asset | null = raw ? { ...raw, type: "stock" } : null;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!user || !selectedStock || !shares) {
        setMessage("Please log in, select a stock, and enter shares.");
        return;
        }

        try {
        await addStockToPortfolio(user, selectedStock, Number(shares));
        setMessage("Stock successfully added to portfolio.");
        setShares('');
        setSelected(null);
        } catch (error) {
        console.error("Error saving:", error);
        setMessage("Failed to add the requested stock.");
        }
    };
    ...
    return (
        ...
        <Autocomplete
            ...
            sx={{ marginBottom: 2 }}
            fullWidth
            // value={selectedStock} // <-- This syncs with selected
            value={selectedStock ? {
